...
namespace starH45.net.mp3
{
	public class AlbumPanel : Control
	{
		...
		private void PaintCurrentAlbumHuuuge(PaintEventArgs e)
		{
			...
			starH45.net.mp3.library.LibraryEntry[] songs = this.Library.QueryLibrary("Album LIKE '" + m_items[m_currentIndex].SongInfo.Album.Replace("'", "''") + "'");
			using (Font f = new Font(Font.FontFamily, 16, FontStyle.Regular, GraphicsUnit.Pixel))
			{
				using (SolidBrush foreColorBrush = new SolidBrush(ForeColor))
				{
					...
					int top = 63;
					foreach (starH45.net.mp3.library.LibraryEntry song in songs)
					{
