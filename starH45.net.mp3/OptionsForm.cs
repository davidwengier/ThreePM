...
namespace starH45.net.mp3
{
	public partial class OptionsForm : BaseForm
	{
		...
		public OptionsForm()
		{
			...
			chkShowTree.Checked = Utilities.GetValue("LibraryForm.ShowTree", true);
			chkShowAlbumArt.Checked = Utilities.GetValue("LibraryForm.ShowAlbumArt", true);
			rdoChangeToNowPlaying.Checked = Utilities.GetValue("LibraryForm.NowPlayingAfterPlay", false);
			chkAutoChangeToPlaylist.Checked = Utilities.GetValue("LibraryForm.PlaylistAfterQueue", false);
			rdoTrackInLibrary.Checked = Utilities.GetValue("LibraryForm.TrackNowPlayingInTree", true);
			chkContributing.Checked = Utilities.GetValue("LibraryForm.TrackContributing", false);

			chkHideIgnoredSongs.Checked = Utilities.GetValue("LibraryForm.HideIgnoredSongs", false);
			...
			rdoByArtist.Checked = Utilities.GetValue("LibraryForm.TrackByArtist", false);
			rdoByAlbum.Checked = Utilities.GetValue("LibraryForm.TrackByAlbum", true);
			txtAlbumArtSize.Text = Utilities.GetValue("LibraryForm.AlbumArtSize", 100).ToString();

			txtCustomRightClickOptionCommand.Text = Utilities.GetValue("General.CustomRightClickOptionCommand", "");
			txtCustomRightClickOptionName.Text = Utilities.GetValue("General.CustomRightClickOptionName", "");
			...
			cboVisualization.Items.Add("Spectrum - starH45.net.mp3");
			...
			cboSkin.SelectedIndex = cboSkin.FindString(Utilities.GetValue("BaseForm.Skin", "Default.skin.xml"));
			...
			txtToasterStayDelay.Text = Utilities.GetValue("ToasterForm.StayDelay", 2000).ToString();
			txtToasterFadeDelay.Text = Utilities.GetValue("ToasterForm.FadeDelay", 50).ToString();

			txtFormatString.Text = player.Player.SongInfoFormatString;
			txtSecondsBeforeUpdatePlayCount.Text = player.Player.SecondsBeforeUpdatePlayCount.ToString();
			chkIgnorePlayed.Checked = player.Player.IgnorePreviouslyPlayedSongsInRandomMode;
			rdoEver.Checked = player.Player.NeverPlayIgnoredSongs;
			rdoRandomly.Checked = !player.Player.NeverPlayIgnoredSongs;
			txtNumRecent.Text = Utilities.GetValue("LibraryForm.SongsToShowInRecentLists", 50).ToString();

			chkEnableAudioScrobbler.Checked = player.Player.AudioscrobblerEnabled;
			txtAudioscrobblerUsername.Text = player.Player.AudioscrobblerUserName;
			txtAudioscrobblerPassword.Text = player.Player.AudioscrobblerPassword;
		...
		protected override void InitLibrary()
		{
			...
			foreach (starH45.net.mp3.library.LibraryEntry entry in Library.GetInternetRadios())
			{
		...
		{
			foreach (string s in player.Player.GetDevices())
			{
		...
		private void LibraryOptions_CheckedChanged(object sender, EventArgs e)
		{
			...
			Utilities.SetValue("LibraryForm.ShowTree", chkShowTree.Checked);
			Utilities.SetValue("LibraryForm.ShowAlbumArt", chkShowAlbumArt.Checked);
			Utilities.SetValue("LibraryForm.NowPlayingAfterPlay", rdoChangeToNowPlaying.Checked);
			Utilities.SetValue("LibraryForm.PlaylistAfterQueue", chkAutoChangeToPlaylist.Checked);
			Utilities.SetValue("LibraryForm.TrackNowPlayingInTree", rdoTrackInLibrary.Checked);
			Utilities.SetValue("LibraryForm.TrackByArtist", rdoByArtist.Checked);
			Utilities.SetValue("LibraryForm.TrackByAlbum", rdoByAlbum.Checked);
			Utilities.SetValue("LibraryForm.HideIgnoredSongs", chkHideIgnoredSongs.Checked);

			Utilities.SetValue("LibraryForm.TrackContributing", chkContributing.Checked);
		...
		private void cboSkin_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Utilities.SetValue("BaseForm.Skin", cboSkin.Text);
		...
		{
			player.Player.SongInfoFormatString = txtFormatString.Text;
			Utilities.SetValue("Player.SongInfoFormatString", txtFormatString.Text);
		}
		...
		private void txtAlbumArtSize_TextChanged(object sender, EventArgs e)
		{
			...
			Utilities.SetValue("LibraryForm.AlbumArtSize", i);
		...
		private void txtToasterStayDelay_TextChanged(object sender, EventArgs e)
		{
			...
			if (i < 10) i = 10;
			Utilities.SetValue("ToasterForm.StayDelay", i);
		}
		...
		private void txtToasterFadeDelay_TextChanged(object sender, EventArgs e)
		{
			...
			if (i < 10) i = 10;
			Utilities.SetValue("ToasterForm.FadeDelay", i);
		}
		...
		private void txtSecondsBeforeUpdatePlayCount_TextChanged(object sender, EventArgs e)
		{
			...
			if (i < 1) i = 1;
			Utilities.SetValue("Player.SecondsBeforeUpdatePlayCount", i);
			player.Player.SecondsBeforeUpdatePlayCount = i;
		}

		private void chkIgnorePlayed_CheckedChanged(object sender, EventArgs e)
		{
			if (m_initialising) return;
			Utilities.SetValue("Player.IgnorePreviouslyPlayedSongsInRandomMode", chkIgnorePlayed.Checked);
			player.Player.IgnorePreviouslyPlayedSongsInRandomMode = chkIgnorePlayed.Checked;
		}
		...
		private void txtNumRecent_TextChanged(object sender, EventArgs e)
		{
			...
			if (i < 1) i = 1;
			Utilities.SetValue("LibraryForm.SongsToShowInRecentLists", i);
		...
		{
			if (m_initialising) return;
			Utilities.SetValue("Player.NeverPlayIgnoredSongs", rdoEver.Checked);
			player.Player.NeverPlayIgnoredSongs = rdoEver.Checked;
		}

		private void txtCustomRightClickOption_TextChanged(object sender, EventArgs e)
		{
			if (m_initialising) return;
			Utilities.SetValue("General.CustomRightClickOptionCommand", txtCustomRightClickOptionCommand.Text);
			Utilities.SetValue("General.CustomRightClickOptionName", txtCustomRightClickOptionName.Text);
		}
		...
		private void btnSaveLyrics_Click(object sender, EventArgs e)
		{
			...
			starH45.net.mp3.library.LibraryEntry[] entries = Library.QueryLibrary("Lyrics <> '' AND Lyrics IS NOT NULL");
			foreach (starH45.net.mp3.library.LibraryEntry entry in entries)
			{
				starH45.net.mp3.utilities.LyricsHelper.SaveLyricsFile(entry as starH45.net.mp3.player.SongInfo, entry.Lyrics);
			}
		...
		{
			if (m_initialising) return;
			Utilities.SetValue("Player.AudioscrobblerEnabled", chkEnableAudioScrobbler.Checked);
			player.Player.AudioscrobblerEnabled = chkEnableAudioScrobbler.Checked;
		}

		private void txtAudioscrobblerUsername_TextChanged(object sender, EventArgs e)
		{
			if (m_initialising) return;
			Utilities.SetValue("Player.AudioscrobblerUserName", txtAudioscrobblerUsername.Text);
			player.Player.AudioscrobblerUserName = txtAudioscrobblerUsername.Text;
		}

		private void txtAudioscrobblerPassword_TextChanged(object sender, EventArgs e)
		{
			if (m_initialising) return;
			Utilities.SetValue("Player.AudioscrobblerPassword", txtAudioscrobblerPassword.Text);
			player.Player.AudioscrobblerPassword = txtAudioscrobblerPassword.Text;
		}
