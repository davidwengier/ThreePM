	...
	public class BaseForm : Form
	{
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			if (checkForMove && (m_captionheight == 0 || e.Y < m_captionheight) && e.Button == MouseButtons.Left)
			{
				...
				{
					Utilities.SetValue(this.Name + ".Show", false);
					this.Close();
				}
		...
		protected override void OnLoad(EventArgs e)
		{
			if (!this.DesignMode)
			{
				...
				if (m_autoSaveLeft)
				{
					int newLeft = Utilities.GetValue(this.Name + ".Left", this.Left);
					this.Left = this.Left < SystemInformation.VirtualScreen.Left ? 0 : newLeft;
				}
				if (m_autoSaveTop)
				{
					int newTop = Utilities.GetValue(this.Name + ".Top", this.Top);
					this.Top = this.Top < SystemInformation.VirtualScreen.Top ? 0 : newTop;
				}
				if (m_sizable || FormBorderStyle == FormBorderStyle.Sizable) this.Width = Utilities.GetValue(this.Name + ".Width", this.Width);
				if (m_sizable || FormBorderStyle == FormBorderStyle.Sizable) this.Height = Utilities.GetValue(this.Name + ".Height", this.Height);
			}
			base.OnLoad(e);
		}
		...
		protected override void Dispose(bool disposing)
		{
			...
			{
				if (m_autoSaveLeft) Utilities.SetValue(this.Name + ".Left", Left);
				if (m_autoSaveTop) Utilities.SetValue(this.Name + ".Top", Top);
				if (m_sizable || FormBorderStyle == FormBorderStyle.Sizable) Utilities.SetValue(this.Name + ".Width", this.Width);
				if (m_sizable || FormBorderStyle == FormBorderStyle.Sizable) Utilities.SetValue(this.Name + ".Height", this.Height);
			}
		...
		public virtual void ReloadSkin(bool loadBlankSkinFirst)
		{
			try
			{
				...
				xmlFile += "Skins\\";
				xmlFile += Utilities.GetValue("BaseForm.Skin", "_no_file_found_");
