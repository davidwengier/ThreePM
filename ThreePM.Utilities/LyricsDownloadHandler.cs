...
namespace ThreePM.utilities
{
	internal class LyricsManiaHandler : ILyricsSiteHandler
	{
		...
		public string GetSearchURL(ThreePM.player.SongInfo song)
		{
		...
		public bool GetLyrics(string htmlPage, out string lyrics)
		{
			...
			{
				string regex = @"<h3>(?<songtitle>.*)</h3>\W*?<table.*?</table>\W*?(?:<br><br>)[\s]*(?<img><img.*?<br><br>)*(?<lyrics>.*?)(?:<br><br>\W*?<b><a)";
		...
		public LyricsSearchResults ProcessSearchResults(ThreePM.player.SongInfo song, string htmlPage, out string nextURL)
		{
			nextURL = "";
			...
			else
			{
				...
				string regex = song.Title.Replace("'", "'*");
				regex = regex.Replace("?", "\\?*");
				regex = regex.Replace(".", "\\.*");
				regex = regex.Replace("*", "\\*");
				regex = regex.Replace("+", "&");
				...
				if (start != -1)
				{
					start = htmlPage.LastIndexOf("http://", start);
					int end = htmlPage.IndexOf("\" title", start);
					if (end != -1)
					{
