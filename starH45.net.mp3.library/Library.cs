...
namespace starH45.net.mp3.library
{
	public class Library : ILibrary, IDisposable
	{
		...
		private Dictionary<int, FileSystemWatcher> m_fileWatchers = new Dictionary<int, FileSystemWatcher>();
		private List<string> m_supportedExtensions = new List<string>();
		private object m_dbSyncObject = new object();
		private string m_connectionString;
		private Thread m_scanningThread;
        private List<DataRow> m_watchFolders = new List<DataRow>();
		private Dictionary<string, bool> m_existingFiles = new Dictionary<string, bool>();
		...
		public int SongCount
		{
			get
			{
                if (m_songCount == -1)
                {
                    lock (m_dbSyncObject)
                    {
                        m_songCount = Convert.ToInt32(SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT COUNT(LibraryID) FROM Library WHERE Deleted = 0"));
                    }
                }
                return m_songCount;
			}
		...
		{
			m_connectionString = "Data Source=" + dbFile + ";Compress=True;New=True;";
		...
		public void Dispose()
		{
			try
			{
                foreach (FileSystemWatcher watcher in m_fileWatchers.Values)
                {
                    watcher.Dispose();
                }
                m_fileWatchers = null;
		...
		public void RefreshLibrary()
		{
			...
			if (m_scanningThread != null && m_scanningThread.IsAlive)
			{
				m_scanningThread.Abort();
				m_scanningThread.Join();
			}
			...
			lock (m_dbSyncObject)
			{
				watch = SQLiteHelper.ExecuteDataSet(m_connectionString, "SELECT WatchFolderID, Folder FROM WatchFolders");
			}

			string crit = "";
            m_watchFolders = new List<DataRow>();
			foreach (DataRow dr in watch.Tables[0].Rows)
			{
				if (Directory.Exists(dr["Folder"].ToString()))
				{
					crit += dr["WatchFolderID"].ToString() + ", ";
					m_watchFolders.Add(dr);
				}
				else
				{
			...
			lock (m_dbSyncObject)
			{
				reader = SQLiteHelper.ExecuteDataSet(m_connectionString, "SELECT Filename FROM Library " + crit);
			}

			//System.Diagnostics.Debug.WriteLine("Loading file dictionary");
            m_existingFiles = new Dictionary<string, bool>();
            foreach (DataRow dr in reader.Tables[0].Rows)
            {
				if (m_existingFiles.ContainsKey(dr["Filename"].ToString()))
				{
					// it must be in the db twice.. delete one of them?
					lock (m_dbSyncObject)
					{
						int id = Convert.ToInt32(SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT LibraryID FROM Library WHERE Filename = '" + dr["Filename"].ToString().Replace("'", "''") + "' LIMIT 1"));
						SQLiteHelper.ExecuteNonQuery(m_connectionString, "DELETE FROM Library WHERE LibraryID = " + id);
					}
				}
				else
				{
					m_existingFiles.Add(dr["Filename"].ToString(), true);
				}
            }
            m_songCount = m_existingFiles.Count;
			OnSongCountChanged();

			//System.Diagnostics.Debug.WriteLine("Creating watchers");
            foreach (DataRow dr in m_watchFolders)
            {
        ...
        public void AddWatchFolder(string dir)
        {
            ...
            lock (m_dbSyncObject)
            {
                i = Convert.ToInt32(SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT COUNT(WatchFolderID) FROM WatchFolders WHERE Folder = '" + dir.Replace("'", "''") + "'"));
            }
            if (i == 0 && Directory.Exists(dir))
            {
                ...
                lock (m_dbSyncObject)
                {
                    SQLiteHelper.ExecuteNonQuery(m_connectionString, "INSERT INTO WatchFolders (Folder) VALUES ('" + dir.Replace("'", "''") + "')");
                    ds = SQLiteHelper.ExecuteDataSet(m_connectionString, "SELECT * FROM WatchFolders WHERE Folder = '" + dir.Replace("'", "''") + "'");

                }
                m_watchFolders.Add(ds.Tables[0].Rows[0]);
        ...
        public void RemoveWatchFolder(string dir)
        {
            ...
            lock (m_dbSyncObject)
            {
                int id = Convert.ToInt32(SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT WatchFolderID FROM WatchFolders WHERE Folder = '" + dir.Replace("'", "''") + "'"));
                SQLiteHelper.ExecuteNonQuery(m_connectionString, "DELETE FROM Library WHERE WatchFolderID = " +id);
                SQLiteHelper.ExecuteNonQuery(m_connectionString, "DELETE FROM WatchFolders WHERE WatchFolderID = " + id);
            }
		...
		private void StartWatching(int WatchFolderID, string dir)
		{
			...
			if (!m_fileWatchers.ContainsKey(WatchFolderID))
			{
				...
				m_fileWatchers.Add(WatchFolderID, watcher);
			}
		}
		...
		{
			int id = -1;
			foreach (int i in m_fileWatchers.Keys)
			{
				if (Object.ReferenceEquals(m_fileWatchers[i], sender))
				{
					id = i;
					break;
				}
		...
		public void SetSupportedExtensions(string[] exts)
		{
			...
			{
				m_supportedExtensions.Add('.' + ext.Substring(ext.IndexOf('.') + 1).ToLower());
			}
		...
		{
			if (m_supportedExtensions.Count == 0)
			{
				throw new ArgumentException("You must set the supported extensions before you can scan for files.");
			}

			m_scanningThread = new Thread(new ThreadStart(InternalScanFolder));
            m_scanningThread.IsBackground = true;
		...
		private void InternalScanFolder()
		{
			...
			try
			{
				for (int i = 0; i < m_watchFolders.Count; i++)
				{
					ScanFolder(m_watchFolders[i]);
				}

				// remove anything left in existingFiles;
				foreach (string s in m_existingFiles.Keys)
				{
					lock (m_dbSyncObject)
					{
						SQLiteHelper.ExecuteNonQuery(m_connectionString, "DELETE FROM Library WHERE Filename = '" + s.Replace("'", "''") + "'");
					}
					m_songCount--;
					OnSongCountChanged();
				}
        ...
        {
			if (m_supportedExtensions.Contains(Path.GetExtension(file).ToLower()))
			{
		...
		public void Update(LibraryEntry entry)
		{
			...
			lock (m_dbSyncObject)
			{
				...
				SQLiteHelper.ExecuteNonQuery(m_connectionString, sql, parameters);
			}
		...
		public void AddInternetRadio(string url)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "INSERT INTO InternetRadio (Url, DateAdded, PlayCount) VALUES (@pUrl, datetime('now', 'localtime'), 0)", parameters);
			}
		}
		...
		public void UpdateInternetRadio(string url, string title)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE InternetRadio SET Title = @pTitle WHERE Url = @pUrl", parameters);
			}
		}
		...
		public void DeleteInternetRadio(string url)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "DELETE FROM InternetRadio WHERE Url = @pUrl", parameters);
			}
		}
        ...
        private void Add(int watchFolderID, string file)
		{
            ...
            lock (m_dbSyncObject)
            {
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "INSERT INTO Library (WatchFolderID, Filename, DateAdded, PlayCount, Ignored, Deleted) VALUES (@pWatchFolderID, @pFileName, datetime('now', 'localtime'), 0, 0, 0)", parameters);
            }
			Update(file);
		}
        ...
        private bool Exists(string file)
		{
            ...
            lock (m_dbSyncObject)
            {
                return Convert.ToBoolean(SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT COUNT(LibraryID) FROM Library WHERE Filename = @pFileName", parameters));
            }
        }
		...
		void ILibrary.UpdatePlayDate(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET DatePlayed = datetime('now', 'localtime') WHERE Filename = @pFileName", parameters);
			}
		}
		...
		void ILibrary.UpdatePlayCount(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				...
				{
					i = SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE InternetRadio SET PlayCount = PlayCount + 1 WHERE Url = @pFileName", parameters);
				}
				else
				{
					i = SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET PlayCount = PlayCount + 1 WHERE Filename = @pFileName", parameters);
				}
			}
			if (i > 0)
			{
		...
		public void Delete(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET Deleted = NOT Deleted WHERE Filename = @pFilename", parameters);
			}
		...
		public void Ignore(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET Ignored = 1 WHERE Filename = @pFilename", parameters);
			}
			OnLibraryUpdated(GetSong(filename) as LibraryEntry);
		}
		...
		public void UnIgnore(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET Ignored = 0 WHERE Filename = @pFilename", parameters);
			}
			OnLibraryUpdated(GetSong(filename) as LibraryEntry);
		}
        ...
        private NeedsUpdateResult NeedsUpdate(string file)
		{
            ...
            lock (m_dbSyncObject)
            {
                o = SQLiteHelper.ExecuteScalar(m_connectionString, "SELECT DateAdded FROM Library WHERE Filename = @pFileName", parameters);
            }
		...
		public int GetPlayCount(string filename)
		{
			...
			lock (m_dbSyncObject)
			{
				o = SQLiteHelper.ExecuteScalar(m_connectionString, sql);
			}
			if (o == null || o == DBNull.Value)
			{
				return 0;
		...
		public LibraryEntry[] GetAlbumsAsEntries()
		{
			...
			lock (m_dbSyncObject)
			{
				ds = SQLiteHelper.ExecuteDataSet(m_connectionString, sql);
			}
...
//            }
		...
		public DataSet GetDataSet(string sql)
		{
			lock (m_dbSyncObject)
			{
				return SQLiteHelper.ExecuteDataSet(m_connectionString, sql);
			}
		}
		...
		public void SetLyrics(string title, string artist, string lyrics)
		{
			...
			lock (m_dbSyncObject)
			{
				i = SQLiteHelper.ExecuteNonQuery(m_connectionString, "UPDATE Library SET Lyrics = @pLyrics WHERE Title LIKE @pTitle AND Artist LIKE @pArtist", parameters);
			}
		}
		...
		public string GetLyrics(string title, string artist)
		{
			...
			lock(m_dbSyncObject)
			{
				lyrics = (SQLiteHelper.ExecuteScalar(m_connectionString, sql, parameters) ?? "").ToString();
			}
			return lyrics;
		}
	}
